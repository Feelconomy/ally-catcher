...
- **기계에 넣기** 토글 — 끄면 인형통에 더 이상 채워지지 않습니다. 이미 뽑은
  인형과 도감은 그대로 두므로 플레이어의 보관함이 비지 않습니다. 목록에서는
  '꺼짐' 배지와 흑백 처리로 표시됩니다. 되돌릴 수 있어 삭제보다 안전합니다
- **인형 추가** — 두 가지 방법으로 그림을 넣습니다
  - **칸별로** — 기본 · 집게에 잡힘 · 떨어짐 · 뽑음 네 칸을 하나씩 눌러 포즈마다 이미지를 고릅니다.
    저장된 인형도 칸 하나만 눌러 그 포즈만 바꿀 수 있습니다. 네 칸이 다 차야 저장됩니다.
  - **붙여넣기** — 이미지를 복사해 ⌘V / Ctrl+V 하면 빈 칸부터 차례로 채워집니다 (다음에 채워질
    칸은 초록 테두리). PC 에서 칸 위에 마우스를 올린 채 붙여넣으면 그 칸을 바꿉니다.
  - **2×2 시트로 한 번에** — 시트 한 장을 고르면 브라우저에서 바로 네 칸으로 잘라 넣습니다
    (`splitSheetFile()`, `tools/split_sheet.py` 와 같은 방식)

  두 방법 모두 투명한 가장자리를 잘라내고 220px webp 로 줄여 저장하며, 인형 하나당 90KB 안팎입니다.
- **포즈 수정** — 기본 인형도 칸을 눌러(또는 붙여넣어) 포즈 그림을 바꿀 수 있습니다. 원래 그림과
  달라진 칸만 저장하고 나머지는 원본 파일을 그대로 씁니다. **그림 원래대로**로 되돌립니다.
- **삭제** — 모든 인형을 지울 수 있습니다 (확인창을 거칩니다). 모든 기계에서 빠지고, 이미 뽑은
  사람의 보관함에서도 사라집니다. 잠깐 빼두기만 하려면 '기계에 넣기'를 끄세요.
  기본 인형은 원본이 `js/data.js` 에 남아 있어 인형 탭 아래 **삭제한 기본 인형**에서 되살릴 수
  있습니다 (기계 구성은 돌아오지 않아 기계 탭에서 다시 넣어야 합니다). 관리자가 추가한 인형은
  완전히 지워집니다.

**화면 탭**

플레이 화면 스킨의 **기본값**을 고릅니다. 사용자는 기계 상세 화면의 '플레이 화면'에서
그린/기본을 직접 고를 수 있고, 고르면 그쪽이 우선합니다 (그 기기에 저장).

- **그린** (기본) — 밝은 초록 캐비닛 · 큰 인형 16마리 무더기 · 아케이드 스틱과 드롭 버튼 분리
- **기본** — 원래의 어두운 캐비닛 · 인형 9마리 · 가로 레버와 전체폭 버튼

두 스킨의 CSS는 각각 `.screen.arcade` / `.screen.classic` 아래에 가둬 뒀습니다.
`.stick` · `.odds` · `.controls` 처럼 같은 이름을 서로 다르게 쓰기 때문에,
스코프를 풀면 두 스킨이 서로를 덮어씁니다.
...
카탈로그(`js/data.js`)는 그대로 두고, 바뀐 필드와 추가한 인형만 따로 쌓아 부팅 때 그
위에 덮어씁니다(`Store.applyAdmin()`). 기본 인형은 원본 사본(`DOLL_BASE`)에서 매번 다시
세운 뒤 수정분을 얹기 때문에, 삭제·그림 교체를 되돌리면 새로고침 없이 바로 원래대로 돌아옵니다. **전부 원래대로 되돌리기**를 누르면 저장분을
비우고 새로고침해 원본으로 돌아갑니다.

##### 서버 저장

관리자가 고친 값은 서비스 전체가 공유하므로 Supabase의 `catalog` 테이블 한 줄
(`id='admin'`)에 통째로 저장합니다. 편집이 드물어 부분 병합 없이 통짜로 주고받고,
**마지막에 저장한 쪽이 이깁니다**. 부팅 때 서버 값을 받아 로컬 위에 덮어씁니다.

처음 한 번은 테이블을 만들어야 합니다 — Supabase 대시보드 → SQL Editor 에
`supabase/catalog.sql` 을 붙여넣고 Run 하세요. 만들기 전에는 저장할 때
'서버 저장 실패' 토스트가 뜨고 그 기기에만 반영됩니다.
...
tools/              포즈 시트 분할 스크립트
assets/             로고(logo.png · 헤더용 logo-mark.png) + 캐릭터 일러스트
```
...
| `--dark` | `#141414` | 결과·광고 화면 |
| `--arc-body` | `#BCE5CB` | 그린 모드 기계 몸통 |
| `--arc-edge` | `#00A650` | 그린 모드 스틱·드롭 버튼 |
...
조작 방식은 스킨에 따라 다릅니다. 그린 모드는 실제 기계처럼 **왼쪽 스틱**과
**오른쪽 드롭 버튼**이 떨어져 있고 (스틱은 밀면 밑동을 축으로 기울었다가 놓으면
중립으로 돌아옵니다), 가운데에 남은
시간, 그 아래 한 줄에 조준한 인형과 이번 판 확률이 뜹니다. 키보드는 ←→ + 스페이스.
크레딧당 집게는 한 번만 내려가고, 잡아도 올리다가 미끄러지거나 옮기다 떨어뜨릴 수 있습니다.

그린 모드의 인형통은 정면에서 본 **인형 무더기**입니다 — 큰 인형(76px) 16마리가 네 줄로
쌓이고, 앞줄이 뒷줄을 가려 뒷줄은 앞 인형 머리 너머로 얼굴만 보이며, 위로 갈수록 좁아져
가운데가 볼록합니다. 집게는 닿는 인형 중 **머리가 가장 높은 인형(더미 꼭대기)부터** 물고,
무더기보다 앞에 그려져 꼭대기 인형의 머리를 무는 모습이 보입니다. 통 높이는 기기에 맞춰 달라지고, 인형은 더미의 실제 높이
(`PILE_H`) 대비로 한꺼번에 줄어 어느 화면에서도 윗줄이 통 밖으로 나가지 않습니다.
그린 모드로 실패하면 실패 화면도 성공 화면과 같은 브랜드 초록(`--green`)으로 나옵니다.
...
마이페이지의 **누적 플레이 · 누적 성공**은 티켓·포인트처럼 서버 플레이어 행에 함께
저장되어 기기를 바꾸거나 다시 로그인해도 이어집니다. 서버에 컬럼이 없으면(아래 SQL
미실행) 예전처럼 그 브라우저에만 남고, 티켓·포인트 저장에는 영향이 없습니다.

```
supabase/stats.sql   — players 에 plays · wins 컬럼 추가 (한 번만 실행)
```
