...
인형과 기계를 도감처럼 카드로 늘어놓고, 카드를 누르면 편집 시트가 열립니다.

**인형 탭**

- 카드에 네 표정(기본 · 집게에 잡힘 · 떨어짐 · 뽑음)이 그대로 보입니다
- 이름 · 등급(등급을 고르면 포인트도 따라감) 수정
- **인형 추가** — 2×2 포즈 시트를 고르면 브라우저에서 바로 잘라 넣습니다
  (`splitSheetFile()`, `tools/split_sheet.py` 와 같은 방식). 각 포즈는 220px
  webp로 줄여 `localStorage`에 담기며 인형 하나당 90KB 안팎입니다.
  추가한 인형만 그림을 바꾸거나 삭제할 수 있습니다.

**기계 탭**

- 카드에 넣어둔 인형이 함께 보입니다
- 이름 · 티켓 비용 · 기본 확률 · 운영/점검
- **넣을 인형 고르기** — 인형을 눌러 켜고 끄면 그 기계의 구성이 됩니다
  (인형통은 자동으로 다시 채워집니다). 원본의 중복 가중치는 사라지고
  고른 인형이 고르게 섞입니다.

카탈로그(`js/data.js`)는 그대로 두고, 바뀐 필드와 추가한 인형만 `localStorage`의
`admin`에 쌓아 부팅 때 그 위에 덮어씁니다(`Store.applyAdmin()`). 서버에는 저장되지
않고, **전부 원래대로 되돌리기**를 누르면 저장분을 비우고 새로고침해 원본으로 돌아갑니다.
