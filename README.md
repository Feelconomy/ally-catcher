...
플레이 화면 스킨을 고릅니다. 다른 화면은 바뀌지 않습니다.

- **아케이드** (기본) — 밝은 초록 캐비닛 · 인형 33마리 · 아케이드 스틱과 드롭 버튼 분리
...
| `--arc-body` | `#BCE5CB` | 아케이드 스킨 기계 몸통 |
| `--arc-edge` | `#00A650` | 아케이드 스킨 스틱·드롭 버튼 |
...
조작 방식은 스킨에 따라 다릅니다. 아케이드 스킨은 실제 기계처럼 **왼쪽 스틱**과
...
인형통은 여섯 줄로 겹쳐 쌓인 33자리이고, 자리마다 좌우로 조금씩 흔들어 격자가 아니라
더미로 보이게 했습니다. 통 높이는 기기에 맞춰 달라지고, 인형은 더미의 실제 높이
(`PILE_H`) 대비로 줄어 어느 화면에서도 뒷줄이 통 밖으로 나가지 않습니다.
