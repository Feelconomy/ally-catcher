...
인형 이름과 기계 설정을 그 자리에서 고칩니다.

- **인형** — 이름 (17종)
- **기계** — 이름 · 티켓 비용 · 기본 확률 · 운영/점검 · 인형통 비우기

카탈로그(`js/data.js`)는 그대로 두고, 바뀐 필드만 `localStorage`의 `admin`에 쌓아
부팅 때 그 위에 덮어씁니다(`Store.applyAdmin()`). 서버에는 저장되지 않고,
**수정한 값 되돌리기**를 누르면 저장분을 비우고 새로고침해 원본으로 돌아갑니다.
